<?php
 ...
 foreach($settings->heating->planning as $strip){
  ...
  if($strip->id==$settings->heating->strip->id){
   ...
   if($strip_pre->percentage>0){$strip_pre->percentage=$strip_pre->percentage-0.5;}
   $strip_pre->hour_start=NULL;
   $strips[]=$strip_pre;
   // build current strip
   $strip_now=clone $strip;
   $strip_now->temperature=NULL;
   $strip_now->color="#666666";
   $strip_now->percentage=0.5;
   $strips[]=$strip_now;
   // build post strip
   ...
   if($strip_pre->percentage<0.5){$strip_post->percentage=$strip_post->percentage-0.5;}
